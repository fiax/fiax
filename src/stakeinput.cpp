...
// The zFIAX block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zFIAX is a hash of the serial
...
{
    //Create an output returning the zFIAX that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZFIAXOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zFIAX output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zFIAX", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZFIAXOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zFIAX output", __func__);
...
{
    CzFIAXTracker* zfiaxTracker = pwallet->zfiaxTracker.get();
    CMintMeta meta;
    if (!zfiaxTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zfiaxTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a FIAX stake is the outpoint
