...
CzFIAXWallet::CzFIAXWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zfiax seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZFIAXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZFIAXSeed_deprecated()) {
                    LogPrintf("%s: Updated zFIAX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zfiax seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zfiax wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzFIAXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZFIAXCount(nCountLastUsed);
    else if (!walletdb.ReadZFIAXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzFIAXWallet::Lock()
{
    seedMaster = 0;
}

void CzFIAXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzFIAXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZFIAX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzFIAXWallet::LoadMintPoolFromDB()
{
...
void CzFIAXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzFIAXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzFIAXWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zfiaxTracker->HasPubcoinHash(pMint.first)) {
...
bool CzFIAXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZFIAX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zfiaxTracker which also adds to database
    pwalletMain->zfiaxTracker->Add(dMint, true);
...
void CzFIAXWallet::SeedToZFIAX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzFIAXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzFIAXWallet::GenerateDeterministicZFIAX(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzFIAXWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzFIAXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
