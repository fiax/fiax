...
#ifndef FIAX_ZFIAXWALLET_H
#define FIAX_ZFIAXWALLET_H
...
public:
    CzFIAXWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZFIAX(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZFIAX(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //FIAX_ZFIAXWALLET_H
