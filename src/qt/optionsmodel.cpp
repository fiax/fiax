...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::FIAX);
    ...
    if (!settings.contains("nAnonymizeFIAXAmount"))
        settings.setValue("nAnonymizeFIAXAmount", 1000);

    nAnonymizeFIAXAmount = settings.value("nAnonymizeFIAXAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeFIAXAmount"))
        SoftSetArg("-anonymizefiaxamount", settings.value("nAnonymizeFIAXAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in fiax.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeFIAXAmount:
            return QVariant(nAnonymizeFIAXAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeFIAXAmount:
            nAnonymizeFIAXAmount = value.toInt();
            settings.setValue("nAnonymizeFIAXAmount", nAnonymizeFIAXAmount);
            emit anonymizeFIAXAmountChanged(nAnonymizeFIAXAmount);
            break;
        case CoinControlFeatures:
