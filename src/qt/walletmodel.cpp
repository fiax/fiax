...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered fiax address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 FIAX.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal fiax:URI (fiax:XyZ...?message=example)
            {
...
static void NotifyzFIAXReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzFIAXReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzFIAXReset.connect(boost::bind(NotifyzFIAXReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzFIAXReset.disconnect(boost::bind(NotifyzFIAXReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zfiaxTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
